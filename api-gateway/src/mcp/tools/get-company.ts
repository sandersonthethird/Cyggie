...
export async function cyggieGetCompany(args: CyggieGetCompanyArgs): Promise<ToolResult> {
  ...
  // Co-investor names from the synced company_investors join (the legacy
  ...
  const coInvestorRows = await db
    ...
    .innerJoin(investorCompany, eq(investorCompany.id, schema.companyInvestors.investorCompanyId))
    .where(
      and(
