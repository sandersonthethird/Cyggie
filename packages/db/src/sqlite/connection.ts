...
/**
 * Run the full ordered SQLite migration chain against `db`. Extracted from
 * getDatabase() so it can be exercised against a fresh in-memory database in
 * tests (e.g. asserting the migrated schema has every column the sync pull-apply
 * INSERTs — the gap that silently broke meeting/contact down-sync until
 * migration 123). Each migration is individually idempotent (guarded by
 * PRAGMA / IF NOT EXISTS checks), so re-running on an already-migrated db is a
 * no-op. Append new migrations to the END of this list, never the middle.
 */
