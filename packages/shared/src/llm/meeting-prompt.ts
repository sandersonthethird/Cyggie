// =============================================================================
// meeting-prompt.ts — canonical meeting-summary prompt assembly.
//
// Single source of truth (T25) for BOTH the desktop summarizer
// (packages/services/src/llm/summarizer.ts → re-exported via
// packages/services/src/llm/templates.ts) and the gateway enhance route
// (api-gateway/src/routes/meetings.ts). Pure — no DB, no Electron, no Node
// APIs — so it imports cleanly into the api-gateway and mobile workspaces.
//
// Before T25 this logic lived in packages/services/src/llm/templates.ts and
// was hand-mirrored by api-gateway/src/templates/meeting-summary-templates.ts
// (substitutePlaceholders). Those two surfaces silently drifted; this module
// collapses them so a meeting summarized on desktop and one summarized on the
// gateway produce byte-identical prompts for the same context.
// =============================================================================

/**
 * The minimal template shape buildPrompt needs. Desktop's MeetingTemplate
 * (with extra fields: id, category, outputFormat, …) and the canonical
 * MeetingTemplate in ./../constants/meeting-templates both satisfy it
 * structurally.
 */
