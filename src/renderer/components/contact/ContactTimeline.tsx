...
export function ContactTimeline({ contactId, hasEmail = true, className }: ContactTimelineProps) {
  ...
  useEffect(() => {
    if (loaded) return
    window.api
      .invoke<ContactTimelineItem[]>(IPC_CHANNELS.CONTACT_TIMELINE, contactId)
      .then((data) => setItems(Array.isArray(data) ? data : []))
      .catch(console.error)
      .finally(() => setLoaded(true))
  }, [contactId, loaded])

  function handleClick(item: ContactTimelineItem) {
  ...
  const handleNoteUpdated = useCallback((note: ContactNote) => {
    setItems((prev) =>
      prev.map((i) =>
