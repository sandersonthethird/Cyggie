...
import { generateMemo } from '@cyggie/services/llm/memo-generator'
...
import type { MemoGenerateMeta, MemoPreflightResult, InvestmentMemoVersion } from '../../shared/types/company'
import { estimateMemoGenContext } from '@cyggie/services/llm/context-size'
...
import { isMemoSectionHeading } from '@cyggie/services/llm/memo/sections'
...
async function runProducerAgent(input: {
  ...
  try {
    ...
    // Return the FULL persisted version (incl. contentMarkdown + real
    // versionNumber) so the renderer's success path can populate
    // memo.latestVersion exactly like the legacy single-call path does.
    // Without contentMarkdown here the viewer renders blank ("No memo yet").
    const fullVersion = memoRepo.getMemoVersion(result.resultVersionId)
    return {
      success: true,
      contentMarkdown: result.assembledMarkdown,
      version: fullVersion ?? {
        // Defensive fallback: synthesize a usable version from what we have so
        // the viewer never goes blank even if the re-read misses.
        id: result.resultVersionId,
        memoId: input.memoId,
        versionNumber: 0,
        contentMarkdown: result.assembledMarkdown,
        structuredJson: null,
        changeNote: 'Generated by producer agent',
